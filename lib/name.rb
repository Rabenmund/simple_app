class Name
  ...
  def self.reload_weighted
    @weighted = nil
  end

  # TODO remove when DB dump is in place

  def self.seed_family
    NAMES.each_index do |index|
      if index.even?
        GermanFamilyName.create!(
          name: NAMES[index],
          weight: NAMES[index+1].to_i/100,
        )
      end
    end
  end
